-- 0001_init（幂等版）：与 database/init.sql 同构。
-- 即使 MySQL 容器首次启动已通过 init.sql 建好表，prisma migrate deploy 也不会失败，
-- 随后由 _prisma_migrations 记录该迁移已应用。
SET NAMES utf8mb4;
...
DROP PROCEDURE IF EXISTS `__grid_repair_add_fk`;
DELIMITER //
CREATE PROCEDURE `__grid_repair_add_fk`(
...
CALL __grid_repair_add_fk('fault_report', 'fault_report_asset_id_fkey',
...
CALL __grid_repair_add_fk('repair_ticket', 'repair_ticket_fault_report_id_fkey',
...
CALL __grid_repair_add_fk('repair_ticket', 'repair_ticket_team_id_fkey',
...
CALL __grid_repair_add_fk('spare_part_usage', 'spare_part_usage_ticket_id_fkey',
...
CALL __grid_repair_add_fk('ticket_event_log', 'ticket_event_log_ticket_id_fkey',
